...
import xyz.nmasnadithya.openweather.utils.Mode
import xyz.nmasnadithya.openweather.utils.setColorFilter
import xyz.nmasnadithya.openweather.utils.ui.getIconDrawableFromDailyWeather
...
class DailyDetailActivity : BaseAppCompatActivity() {
  companion object {
    const val ITEM_KEY =
         "${BuildConfig.APPLICATION_ID}.ui.main.fivedayforecast.daily_detail_activity_item"
  }
      ...
      adapter = object : RecyclerView.Adapter<VH>() {
        ...
        override fun onBindViewHolder(holder: VH, position: Int) =
            holder.bind(list[position], item.colors.second).also {
              Timber.d("Bind $position ${list[position]}")
            }
      }
